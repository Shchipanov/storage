...
export class Upload {
  constructor(uploadBlock, options = {}) {
    ...
    this._maxFileSize = this._options.maxFileSize
      ? this._options.maxFileSize
      : false;
    this._maxFullSize = this._options.maxFullSize
      ? this._options.maxFullSize
      : false;
    ...
    this._uploadLength = this._options.uploadLength
      ? this._options.uploadLength
      : 1;
    this._accept = this._options.accept ? this._options.accept : false;
    this._emptyMessage = this._options.emptyMessage
      ? this._options.emptyMessage
      : false;
    this._errorMessage = this._options.errorMessage
      ? this._options.errorMessage
      : false;
    this._successMessage = this._options.successMessage
      ? this._options.successMessage
      : false;
    this._input = this._uploadBlock.querySelector('input');
    this._dropZoneBlock = this._dropZone
      ? this._uploadBlock.querySelector('[data-drop-zone]')
      : false;
  ...
  _renderFiles() {
    ...
    error = this._maxFullSize
      ? this._checkFullSize(this._files) > this._maxFullSize
      : false;
    ...
    this._files.forEach((file) => {
      const fileSizeError = this._maxFileSize
      ...
      if (this._previewBlock) {
        reader.addEventListener('load', (readerEvent) => {
          ...
          document.querySelector('.file-input__placeholder').style.color =
            '#fcfcfc';
        });
      }
  ...
  _onPreviewBlockClick(event) {
    ...
    this._overwriteFileList();
    this._previewBlock
        .querySelector(`[data-file-name="${name}"]`)
        .parentElement.remove();
    if (this._dropZoneBlock) {
      this._changeDropZoneBlock();
    }
    const fullSizeError = this._maxFullSize
      ? this._checkFullSize(this._files) > this._maxFullSize
      : false;
    if (fullSizeError) {
      this._uploadBlock.classList.add('is-invalid');
  ...
  _onDropZoneBlockClick(event) {
    if (
      ...
      event.target.closest('.file-input__preview')
    ) {
      return;
    }
    this._input.click();
  ...
  _onDropZoneBlockDrop(event) {
    ...
    this._files = [...this._files, ...event.dataTransfer.files].slice(
        0,
        this._uploadLength
    );
    this._renderFiles();
  ...
  _onInputChange(event) {
    ...
    this._files = [...this._files, ...event.target.files].slice(
        0,
        this._uploadLength
    );
    this._renderFiles();
  ...
  _init() {
    ...
    if (this._dropZoneBlock) {
      this._previewBlock = createElement(
          createPreviewContainerMarkup(this._options)
      );
      this._previewBlock.addEventListener('click', this._onPreviewBlockClick);
      this._dropZoneBlock.addEventListener(
          'dragover',
          this._onDropZoneBlockDragover
      );
      this._dropZoneBlock.addEventListener(
          'dragenter',
      ...
      this._dropZoneBlock.addEventListener(
          'dragleave',
          this._onDropZoneBlockDragleave
      );
